...
# 第 10 课｜建立 Megakernel 性能账本

![第 10 课：建立 Megakernel 性能账本](./lesson-10.png)

> 把总延迟拆成计算、搬运、同步、空闲、调度和 launch，判断普通融合、Graph 或 resident kernel 谁更合适。

## 零基础先看这里

- **它在解决什么：**优化省下的时间为什么不一定等于收益？
- **把它想成：**像记家庭账，少交一笔费用，也可能新增运输费和管理费。
- **这次先不用懂：**可先忽略逐周期归因和功耗统计。

## 本课结论与证据状态

- **一句话结论：**任何优化都必须同时记下省掉的成本和新增的成本。
- **证据状态：**DECISION FRAMEWORK · MIXED EVIDENCE
...
1. 先不看上文，用自己的话回答：优化省下的时间为什么不一定等于收益？
2. 再对照本课结论：任何优化都必须同时记下省掉的成本和新增的成本。
3. 根据 `DECISION FRAMEWORK · MIXED EVIDENCE`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/performance-ledger/)
- [← 上一课 · 第 9 课：Megakernel 怎样连续生成 Token](../lesson09/)
- [下一课 · 第 11 课：手把手读懂一份 B200 NCU 报告 →](../lesson11/)
