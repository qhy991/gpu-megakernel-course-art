...
![第 27 课：Ready 不是一个数字](./lesson-27.png)

> 区分原子计数完整性、payload 可见性、代际身份和安全复用四层含义。

## 零基础先看这里

- **它在解决什么：**计数达到目标，为什么还不能断定数据可用？
- **把它想成：**快递数目对了，不代表箱内货物正确、属于本批，也不代表货架能立即清空。
- **这次先不用懂：**可先忽略原子操作的内存顺序和 epoch 编码。

## 本课结论与证据状态

- **一句话结论：**计数器到达只证明事件数量，不自动证明数据、epoch 和生命周期。
- **证据状态：**SOURCE + MEMORY-MODEL GAP
...
1. 先不看上文，用自己的话回答：计数达到目标，为什么还不能断定数据可用？
2. 再对照本课结论：计数器到达只证明事件数量，不自动证明数据、epoch 和生命周期。
3. 根据 `SOURCE + MEMORY-MODEL GAP`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/ready-is-not-counter/)
- [← 上一课 · 第 26 课：一个 Partial Attention 到底在等什么？](../lesson26/)
- [下一课 · 第 28 课：先 Ready，再入队 →](../lesson28/)
