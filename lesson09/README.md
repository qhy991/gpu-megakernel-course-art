...
# 第 9 课｜Megakernel 怎样连续生成 Token

![第 9 课：Megakernel 怎样连续生成 Token](./lesson-09.png)

> 区分层内 CTA 常驻、每 token 一次 whole-model kernel 与跨 token 不退出的 decode engine，并说明 MPS 的位置。

## 零基础先看这里

- **它在解决什么：**常驻 GPU 的程序怎样连续处理多个 token？
- **把它想成：**像值夜班工人留在工位，订单来了就接着做，不必每次重新报到。
- **这次先不用懂：**可先忽略跨 token 队列的内存一致性细节。

## 本课结论与证据状态

- **一句话结论：**Persistent 描述生命周期；是否跨 token 常驻必须单独说明。
- **证据状态：**SOURCE-AUDITED · LIFETIME MODEL
...
1. 先不看上文，用自己的话回答：常驻 GPU 的程序怎样连续处理多个 token？
2. 再对照本课结论：Persistent 描述生命周期；是否跨 token 常驻必须单独说明。
3. 根据 `SOURCE-AUDITED · LIFETIME MODEL`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/persistent-across-tokens/)
- [← 上一课 · 第 8 课：一层 Llama 在 Megakernel 里怎样流动](../lesson08/)
- [下一课 · 第 10 课：建立 Megakernel 性能账本 →](../lesson10/)
