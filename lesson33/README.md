...
# 第 33 课｜双 CTA Poison Litmus

![第 33 课：双 CTA Poison Litmus](./lesson-33.png)

> 把 poison 实验拆成 Host 准入、共享状态、Producer、Consumer 和 ACK 五层伪代码。

## 零基础先看这里

- **它在解决什么：**怎样用最小实验确认两个执行者真的同步正确？
- **把它想成：**像实验室控制变量：固定交接顺序，再故意拆掉安全锁，看错误是否必然出现。
- **这次先不用懂：**可先忽略伪代码的缓冲区布局和启动参数。

## 本课结论与证据状态

- **一句话结论：**Litmus 必须锁死执行顺序，才能区分真正的同步保证与偶然成功。
- **证据状态：**DIAGNOSTIC PSEUDOCODE · NOT IMPLEMENTED
...
1. 先不看上文，用自己的话回答：怎样用最小实验确认两个执行者真的同步正确？
2. 再对照本课结论：Litmus 必须锁死执行顺序，才能区分真正的同步保证与偶然成功。
3. 根据 `DIAGNOSTIC PSEUDOCODE · NOT IMPLEMENTED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/two-cta-litmus/)
- [← 上一课 · 第 32 课：用 Poison 抓出同步 Bug](../lesson32/)
- [下一课 · 第 34 课：从源码到 B200 的四层证据链 →](../lesson34/)
