...
# 第 41 课｜一层 Llama 到底有几条切缝？

![第 41 课：一层 Llama 到底有几条切缝？](./llama1b_one_layer_island_seams_16x9.png)

> 沿 QKV、Attention、OProj、UpGate、DownProj 和 LMHead，逐条盘点跨边界状态。

## 零基础先看这里

- **它在解决什么：**一层模型应该从哪里切成几个 kernel？
- **把它想成：**接力赛在哪里换棒，不看选手名字，而看交接时要传什么、传多少、会不会停下来。
- **这次先不用懂：**可先忽略张量字节数和 IType 的源码映射。

## 本课结论与证据状态

- **一句话结论：**决定切不切的，不是算子名字，而是边界上必须保存什么。
- **证据状态：**SOURCE-PROVEN · CANONICAL C473
...
1. 先不看上文，用自己的话回答：一层模型应该从哪里切成几个 kernel？
2. 再对照本课结论：决定切不切的，不是算子名字，而是边界上必须保存什么。
3. 根据 `SOURCE-PROVEN · CANONICAL C473`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/lesson-41-one-layer-seams/)
- [← 上一课 · 第 40 课：最大融合一定最好吗？从 Megakernel 到 Island](../lesson40/)
- [下一课 · 第 42 课：四臂实验：怎样给 Island 一个公平位置？ →](../lesson42/)
