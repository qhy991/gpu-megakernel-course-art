...
# 第 17 课｜时间花在哪里，不等于时间能省多少

![第 17 课：时间花在哪里，不等于时间能省多少](./lesson-17.png)

> 从真实 decode-floor 归因区分可见耗时、可消除耗时与 Amdahl 上限。

## 零基础先看这里

- **它在解决什么：**为什么最耗时的 kernel 不一定最值得融合？
- **把它想成：**账单中最大的一项不代表能整项免单，真正可省的常常只是一部分。
- **这次先不用懂：**可先忽略 Amdahl 定律的正式推导。

## 本课结论与证据状态

- **一句话结论：**Profiler 中显眼的 kernel 不等于可以被完整删除的时间。
- **证据状态：**MEASURED ARCHIVE · ATTRIBUTION BOUNDED
...
1. 先不看上文，用自己的话回答：为什么最耗时的 kernel 不一定最值得融合？
2. 再对照本课结论：Profiler 中显眼的 kernel 不等于可以被完整删除的时间。
3. 根据 `MEASURED ARCHIVE · ATTRIBUTION BOUNDED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/attribution-vs-savings/)
- [← 上一课 · 第 16 课：不先相信 RESULT.md：自己复算一次 B200 实验](../lesson16/)
- [下一课 · 第 18 课：把 128 KiB 大门拆成 8 个 Page-ready 小门 →](../lesson18/)
