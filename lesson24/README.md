...
![第 24 课：先算能住多少，再画谁等谁](./lesson-24.png)

> 把 exact occupancy 资源账本与 wait-for graph 结合，系统检查跨 CTA 等待是否可能闭环。

## 零基础先看这里

- **它在解决什么：**怎样在运行前判断多个任务会不会互相堵死？
- **把它想成：**先数房间能住几人，再画出谁在等谁；座位不足或等待成环都危险。
- **这次先不用懂：**可先忽略寄存器和共享内存的精确容量计算。

## 本课结论与证据状态

- **一句话结论：**容量账本回答能住多少，等待图回答住进去后是否互相卡住。
- **证据状态：**SOURCE-MODELED · OCCUPANCY CONTRACT
...
1. 先不看上文，用自己的话回答：怎样在运行前判断多个任务会不会互相堵死？
2. 再对照本课结论：容量账本回答能住多少，等待图回答住进去后是否互相卡住。
3. 根据 `SOURCE-MODELED · OCCUPANCY CONTRACT`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/capacity-wait-for-graph/)
- [← 上一课 · 第 23 课：同步写对了，为什么程序还会卡死？](../lesson23/)
- [下一课 · 第 25 课：Role-fluid 还不够，必须 Ready-aware →](../lesson25/)
