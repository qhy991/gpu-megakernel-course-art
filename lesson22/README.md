...
# 第 22 课｜为什么一个 Ready Bit 不够？

![第 22 课：为什么一个 Ready Bit 不够？](./lesson-22.png)

> 解释 instruction ring 重用时的 epoch、ABA、ACK 和 backpressure，避免新旧代混淆。

## 零基础先看这里

- **它在解决什么：**同一个存储位置反复使用时，怎样分清新旧数据？
- **把它想成：**只写“包裹已到”不够，还要写第几批，并等收件确认后才能清空货架。
- **这次先不用懂：**可先忽略 ABA 名称、epoch 位宽和环形槽布局。

## 本课结论与证据状态

- **一句话结论：**Ready 只证明本代可读，ACK 才保护槽位何时可复用。
- **证据状态：**SOURCE-PROVEN · LIFETIME MODEL
...
1. 先不看上文，用自己的话回答：同一个存储位置反复使用时，怎样分清新旧数据？
2. 再对照本课结论：Ready 只证明本代可读，ACK 才保护槽位何时可复用。
3. 根据 `SOURCE-PROVEN · LIFETIME MODEL`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/epoch-aba-ring/)
- [← 上一课 · 第 21 课：Flag 到了，数据就一定到了吗？](../lesson21/)
- [下一课 · 第 23 课：同步写对了，为什么程序还会卡死？ →](../lesson23/)
