...
# 第 46 课｜怎样让只有真正需要的 IType 才申请 TMEM？

![第 46 课：怎样让只有真正需要的 IType 才申请 TMEM？](./tmem_capability_manifest_16x9.png)

> 设计 fail-closed 的 kernel resource manifest：Llama-1B 声明 0 列，真正 tcgen05 IType 显式声明 256/512 列。

## 零基础先看这里

- **它在解决什么：**怎样只让真正需要的任务预订专用空间？
- **把它想成：**每种任务先交资源购物清单，再由编译器汇总；漏写必需品就拒绝开工。
- **这次先不用懂：**可先忽略 TMEM、Python 类型和 JIT 缓存键。

## 本课结论与证据状态

- **一句话结论：**资源能力必须由编译期合同声明，漏标要编译失败。
- **证据状态：**PROPOSED · SOURCE-MAPPED
...
1. 先不看上文，用自己的话回答：怎样只让真正需要的任务预订专用空间？
2. 再对照本课结论：资源能力必须由编译期合同声明，漏标要编译失败。
3. 根据 `PROPOSED · SOURCE-MAPPED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/lesson-46-tmem-manifest/)
- [← 上一课 · 第 45 课：第三把锁：TMEM 为什么能让 occupancy 从 2 变 1？](../lesson45/)
- [下一课 · 第 47 课：Peak-live：为什么资源不能把百分比相加？ →](../lesson47/)
