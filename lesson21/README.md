...
# 第 21 课｜Flag 到了，数据就一定到了吗？

![第 21 课：Flag 到了，数据就一定到了吗？](./lesson-21.png)

> 用跨 CTA producer-consumer 解释为什么原子 flag 正确不等于 payload 已可见。

## 零基础先看这里

- **它在解决什么：**看到“完成”标志时，数据为什么可能还没准备好？
- **把它想成：**厨师要先把菜放到窗口再按铃；服务员听见铃后，才能放心取菜。
- **这次先不用懂：**可先忽略 release/acquire 对应的指令和语法。

## 本课结论与证据状态

- **一句话结论：**完成信号必须在数据之后 release，读取数据必须在看到信号后 acquire。
- **证据状态：**SOURCE + HISTORICAL CORRECTNESS EVIDENCE
...
1. 先不看上文，用自己的话回答：看到“完成”标志时，数据为什么可能还没准备好？
2. 再对照本课结论：完成信号必须在数据之后 release，读取数据必须在看到信号后 acquire。
3. 根据 `SOURCE + HISTORICAL CORRECTNESS EVIDENCE`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/release-acquire-publication/)
- [← 上一课 · 第 20 课：Dynamic Tail：最后半块不做整块工作](../lesson20/)
- [下一课 · 第 22 课：为什么一个 Ready Bit 不够？ →](../lesson22/)
