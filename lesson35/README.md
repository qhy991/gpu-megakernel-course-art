...
# 第 35 课｜实验档案袋：怎样保存一个可复核结论

![第 35 课：实验档案袋：怎样保存一个可复核结论](./lesson-35.png)

> 把源码、编译参数、binary hash、正确性、profile 和计时装进统一 manifest。

## 零基础先看这里

- **它在解决什么：**怎样保存一次实验，才能让别人复查结论？
- **把它想成：**像整理案件档案：样品编号只是起点，还要保存过程、条件和检测结果。
- **这次先不用懂：**可先忽略 manifest 字段格式和哈希工具命令。

## 本课结论与证据状态

- **一句话结论：**有 SHA 只证明文件身份；完整档案才能证明结论身份。
- **证据状态：**EVIDENCE MANIFEST · WORKED EXAMPLE
...
1. 先不看上文，用自己的话回答：怎样保存一次实验，才能让别人复查结论？
2. 再对照本课结论：有 SHA 只证明文件身份；完整档案才能证明结论身份。
3. 根据 `EVIDENCE MANIFEST · WORKED EXAMPLE`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/experiment-envelope/)
- [← 上一课 · 第 34 课：从源码到 B200 的四层证据链](../lesson34/)
- [下一课 · 第 36 课：Page-ready：为什么 128 KiB 大门会让 Megakernel 空等？ →](../lesson36/)
