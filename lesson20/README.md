...
# 第 20 课｜Dynamic Tail：最后半块不做整块工作

![第 20 课：Dynamic Tail：最后半块不做整块工作](./lesson-20.png)

> 解释 DownProj 的 2048 短尾为何不应继续使用满 4096 的 TMA、warp 和 reduction 几何。

## 零基础先看这里

- **它在解决什么：**最后一小段数据，为什么不该按整块规模处理？
- **把它想成：**像货车只剩半车货，就不必再派满编装卸队；按实际货量安排人手更省事。
- **这次先不用懂：**可先忽略 TMA、warp 和 HMMA 的具体执行参数。

## 本课结论与证据状态

- **一句话结论：**尾块变短时，执行几何也应缩短，但整步收益受占比限制。
- **证据状态：**MEASURED · LEGACY INCREMENTAL WIN
...
1. 先不看上文，用自己的话回答：最后一小段数据，为什么不该按整块规模处理？
2. 再对照本课结论：尾块变短时，执行几何也应缩短，但整步收益受占比限制。
3. 根据 `MEASURED · LEGACY INCREMENTAL WIN`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/dynamic-tail/)
- [← 上一课 · 第 19 课：Split-KV：把 8 个长任务拆成 128 个短任务](../lesson19/)
- [下一课 · 第 21 课：Flag 到了，数据就一定到了吗？ →](../lesson21/)
