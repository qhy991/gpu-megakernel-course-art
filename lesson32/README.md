...
# 第 32 课｜用 Poison 抓出同步 Bug

![第 32 课：用 Poison 抓出同步 Bug](./lesson-32.png)

> 设计毒值、延迟门和故障签名，让缺失 TMA 等待或 release/acquire 的错误稳定暴露。

## 零基础先看这里

- **它在解决什么：**偶尔才出现的同步错误，怎样让它稳定暴露？
- **把它想成：**像在漏水处撒荧光粉并加压；只要管道有缝，痕迹就更容易被看见。
- **这次先不用懂：**可先忽略毒值选择、延迟门参数和 TMA 指令。

## 本课结论与证据状态

- **一句话结论：**正例全绿不够；错误实现必须被测试稳定杀死。
- **证据状态：**DIAGNOSTIC DESIGN · UNMEASURED
...
1. 先不看上文，用自己的话回答：偶尔才出现的同步错误，怎样让它稳定暴露？
2. 再对照本课结论：正例全绿不够；错误实现必须被测试稳定杀死。
3. 根据 `DIAGNOSTIC DESIGN · UNMEASURED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/poison-sync-bugs/)
- [← 上一课 · 第 31 课：把 CPU Ready 模型翻译成 CUDA Controller](../lesson31/)
- [下一课 · 第 33 课：双 CTA Poison Litmus →](../lesson33/)
