...
# 第 50 课｜2 CTA/SM 要打开哪五把锁？

![第 50 课：2 CTA/SM 要打开哪五把锁？](./five_locks_for_two_cta_16x9-v2.png)

> Shared、register、TMEM、grid supply、worker identity 五个条件必须同时成立。

## 零基础先看这里

- **它在解决什么：**资源够放两组任务，为什么它们仍可能跑不起来？
- **把它想成：**厨房放得下两位厨师还不够：还要有两份订单、两套材料，并让两人各自认领任务。
- **这次先不用懂：**可先忽略 CTA、B200 容量和 cluster2。

## 本课结论与证据状态

- **一句话结论：**occupancy=2 只是第二把门后的通行证，不是最终结果。
- **证据状态：**SOURCE + HISTORICAL NEGATIVE CONTROLS
...
1. 先不看上文，用自己的话回答：资源够放两组任务，为什么它们仍可能跑不起来？
2. 再对照本课结论：occupancy=2 只是第二把门后的通行证，不是最终结果。
3. 根据 `SOURCE + HISTORICAL NEGATIVE CONTROLS`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/lesson-50-five-locks-two-cta/)
- [← 上一课 · 第 49 课：两岛 P0：怎样把一个想法变成可证伪候选？](../lesson49/)
- [下一课 · 第 51 课：R0–R5：五个实验臂为什么不能跳级？ →](../lesson51/)
