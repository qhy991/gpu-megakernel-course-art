...
# 第 52 课｜三种 Pipeline：为什么一个数字 3 会指三件不同的事？

![第 52 课：三种 Pipeline：为什么一个数字 3 会指三件不同的事？](./r1_pipeline_depth_vs_pages_16x9.png)

> 区分外层 instruction pipeline、matvec weight-input pipeline 与 output scratch pipeline。

## 零基础先看这里

- **它在解决什么：**为什么三个都叫 pipeline，却不能一起修改？
- **把它想成：**餐厅的订单队列、烹饪托盘和装盘区是三套周转系统；数量碰巧相同，也不是同一件事。
- **这次先不用懂：**可先忽略 mbarrier parity、页编号和取模方式。

## 本课结论与证据状态

- **一句话结论：**改 input depth 时，output scratch 与 instruction ring 不应该跟着变。
- **证据状态：**SOURCE-PROVEN · R1A PROPOSED
...
1. 先不看上文，用自己的话回答：为什么三个都叫 pipeline，却不能一起修改？
2. 再对照本课结论：改 input depth 时，output scratch 与 instruction ring 不应该跟着变。
3. 根据 `SOURCE-PROVEN · R1A PROPOSED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/lesson-52-pipeline-depth-vs-pages/)
- [← 上一课 · 第 51 课：R0–R5：五个实验臂为什么不能跳级？](../lesson51/)
- [下一课 · 第 53 课：三页以后还能流水吗？把 64 KiB 大门拆成两扇 →](../lesson53/)
