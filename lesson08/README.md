...
# 第 8 课｜一层 Llama 在 Megakernel 里怎样流动

![第 8 课：一层 Llama 在 Megakernel 里怎样流动](./lesson-08.png)

> 把玩具队列映射到真实 Llama 层，区分片上直通、跨 CTA 发布和必须落全局内存的边。

## 零基础先看这里

- **它在解决什么：**Llama 一层的数据为什么不能随便融合？
- **把它想成：**像接力赛，先看接力棒由谁保管、何时交接，再决定谁能并组。
- **这次先不用懂：**可先忽略张量的精确形状和数学公式。

## 本课结论与证据状态

- **一句话结论：**融合边界应跟随数据所有权，而不是跟随算子名字。
- **证据状态：**SOURCE-AUDITED · CANONICAL + LEGACY
...
1. 先不看上文，用自己的话回答：Llama 一层的数据为什么不能随便融合？
2. 再对照本课结论：融合边界应跟随数据所有权，而不是跟随算子名字。
3. 根据 `SOURCE-AUDITED · CANONICAL + LEGACY`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/llama-layer-dataflow/)
- [← 上一课 · 第 7 课：亲手搭一个最小 Megakernel](../lesson07/)
- [下一课 · 第 9 课：Megakernel 怎样连续生成 Token →](../lesson09/)
