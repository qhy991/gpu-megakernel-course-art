...
# 第 31 课｜把 CPU Ready 模型翻译成 CUDA Controller

![第 31 课：把 CPU Ready 模型翻译成 CUDA Controller](./lesson-31.png)

> 把 Atom→Group→Worker→ACK 状态机映射到 CUDA 原子、release/acquire 和 epoch 生命周期。

## 零基础先看这里

- **它在解决什么：**GPU 上的多个执行者怎样安全交接任务和数据？
- **把它想成：**像接力赛：先交棒，再确认对方接到，最后才能把这根棒交给下一轮。
- **这次先不用懂：**可先忽略 CUDA 原子 API 和具体内存顺序指令。

## 本课结论与证据状态

- **一句话结论：**relaxed 用于找路，release/acquire 用于数据交接，ACK 用于代际复用。
- **证据状态：**CUDA CONTROLLER PROPOSAL · SOURCE-MAPPED
...
1. 先不看上文，用自己的话回答：GPU 上的多个执行者怎样安全交接任务和数据？
2. 再对照本课结论：relaxed 用于找路，release/acquire 用于数据交接，ACK 用于代际复用。
3. 根据 `CUDA CONTROLLER PROPOSAL · SOURCE-MAPPED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/cuda-controller-memory-order/)
- [← 上一课 · 第 30 课：一个 Worker 也能跑完整张 DAG](../lesson30/)
- [下一课 · 第 32 课：用 Poison 抓出同步 Bug →](../lesson32/)
