...
# 第 11 课｜手把手读懂一份 B200 NCU 报告

![第 11 课：手把手读懂一份 B200 NCU 报告](./lesson-11.png)

> 按 duration、occupancy、eligible warp、stall 和源码位置逐层诊断 one-layer Megakernel。

## 零基础先看这里

- **它在解决什么：**工人明明很多，为什么机器仍可能闲着？
- **把它想成：**工人都在厂里不等于都能开工，他们可能正等材料或等信号。
- **这次先不用懂：**可先忽略全部 stall 分类和冷门计数器。

## 本课结论与证据状态

- **一句话结论：**Resident warp 多不等于能发射；报告必须回到等待位置。
- **证据状态：**HISTORICAL NCU SNAPSHOT · RAW REPORT UNAVAILABLE
...
1. 先不看上文，用自己的话回答：工人明明很多，为什么机器仍可能闲着？
2. 再对照本课结论：Resident warp 多不等于能发射；报告必须回到等待位置。
3. 根据 `HISTORICAL NCU SNAPSHOT · RAW REPORT UNAVAILABLE`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/read-b200-ncu/)
- [← 上一课 · 第 10 课：建立 Megakernel 性能账本](../lesson10/)
- [下一课 · 第 12 课：从 NCU 追到 PTX 和 SASS →](../lesson12/)
