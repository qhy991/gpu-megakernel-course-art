...
# 第 12 课｜从 NCU 追到 PTX 和 SASS

![第 12 课：从 NCU 追到 PTX 和 SASS](./lesson-12.png)

> 连接源码、PTX、SASS 与 profiler，解释寄存器 spill、local load/store 和最小证伪实验。

## 零基础先看这里

- **它在解决什么：**为什么看过源码还要追到机器指令？
- **把它想成：**菜谱写的是计划，厨师实际做出的动作才是机器真正执行的内容。
- **这次先不用懂：**可先忽略指令编码和反汇编语法。

## 本课结论与证据状态

- **一句话结论：**只有 exact binary 能把源码解释和硬件指令闭合起来。
- **证据状态：**SOURCE + HISTORICAL BINARY EVIDENCE
...
1. 先不看上文，用自己的话回答：为什么看过源码还要追到机器指令？
2. 再对照本课结论：只有 exact binary 能把源码解释和硬件指令闭合起来。
3. 根据 `SOURCE + HISTORICAL BINARY EVIDENCE`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/ncu-to-ptx-sass/)
- [← 上一课 · 第 11 课：手把手读懂一份 B200 NCU 报告](../lesson11/)
- [下一课 · 第 13 课：下一轮 Megakernel 实验路线图 →](../lesson13/)
