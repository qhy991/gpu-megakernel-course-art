...
# 第 40 课｜最大融合一定最好吗？从 Megakernel 到 Island

![第 40 课：最大融合一定最好吗？从 Megakernel 到 Island](./maximum_fusion_vs_megakernel_islands_16x9-v3.png)

> 用同体 Graph 对照和负向切分控制，理解融合收益与资源包络成本之间的 U 型关系。

## 零基础先看这里

- **它在解决什么：**把所有计算塞进一个大 kernel，一定最快吗？
- **把它想成：**所有工序挤进一间厨房，传菜少了，却可能被最大设备占满；分成几间厨房有时更顺。
- **这次先不用懂：**可先忽略 Graph provider 和历史计时数据。

## 本课结论与证据状态

- **一句话结论：**融合不是越多越好，切分也不会自动让资源变小。
- **证据状态：**MIXED EVIDENCE · ISLANDS UNMEASURED
...
1. 先不看上文，用自己的话回答：把所有计算塞进一个大 kernel，一定最快吗？
2. 再对照本课结论：融合不是越多越好，切分也不会自动让资源变小。
3. 根据 `MIXED EVIDENCE · ISLANDS UNMEASURED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/lesson-40-fusion-vs-islands/)
- [← 上一课 · 第 39 课：Resident、Eligible、Issue：31.25% occupancy 到底说明什么？](../lesson39/)
- [下一课 · 第 41 课：一层 Llama 到底有几条切缝？ →](../lesson41/)
