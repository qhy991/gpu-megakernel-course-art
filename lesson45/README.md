...
# 第 45 课｜第三把锁：TMEM 为什么能让 occupancy 从 2 变 1？

![第 45 课：第三把锁：TMEM 为什么能让 occupancy 从 2 变 1？](./third_lock_tmem_16x9.png)

> TMEM 是独立的硬件准入资源；canonical Llama-1B 甚至申请了 512 列，却没有用它存数值 tile。

## 零基础先看这里

- **它在解决什么：**明明没用一块专用空间，为什么它仍会挡住新任务？
- **把它想成：**停车位虽然空着，但已被预约；别人看到名额被占，就不能把车开进来。
- **这次先不用懂：**可先忽略 TMEM、tcgen05 和二进制元数据。

## 本课结论与证据状态

- **一句话结论：**资源是否被数值代码使用，与它是否影响 kernel 准入，是两件事。
- **证据状态：**SOURCE + HISTORICAL EXACT BINARY
...
1. 先不看上文，用自己的话回答：明明没用一块专用空间，为什么它仍会挡住新任务？
2. 再对照本课结论：资源是否被数值代码使用，与它是否影响 kernel 准入，是两件事。
3. 根据 `SOURCE + HISTORICAL EXACT BINARY`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/lesson-45-tmem-lock/)
- [← 上一课 · 第 44 课：把 7 页改成 3 页，为什么 Shared Memory 可能一字节没降？](../lesson44/)
- [下一课 · 第 46 课：怎样让只有真正需要的 IType 才申请 TMEM？ →](../lesson46/)
