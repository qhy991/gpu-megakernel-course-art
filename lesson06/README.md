...
![第 6 课：从源码识别三种融合](./lesson-06.png)

> 沿 Python 模型、图编译、调度器、指令队列和 resident kernel 区分代码融合、launch 融合与数据流融合。

## 零基础先看这里

- **它在解决什么：**代码写在一起，就等于真正融合了吗？
- **把它想成：**把多张订单订在一起，不代表厨房只开一次火或不再搬食材。
- **这次先不用懂：**可先忽略 PTX、SASS 的逐条指令对应。

## 本课结论与证据状态

- **一句话结论：**源码放在一起、物理只 launch 一次和数据留在片上，是三件不同的事。
- **证据状态：**SOURCE-AUDITED · MULTIPLE SNAPSHOTS
...
1. 先不看上文，用自己的话回答：代码写在一起，就等于真正融合了吗？
2. 再对照本课结论：源码放在一起、物理只 launch 一次和数据留在片上，是三件不同的事。
3. 根据 `SOURCE-AUDITED · MULTIPLE SNAPSHOTS`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/identify-fusion-in-source/)
- [← 上一课 · 第 5 课：怎样准确介绍你的 Megakernel 工作](../lesson05/)
- [下一课 · 第 7 课：亲手搭一个最小 Megakernel →](../lesson07/)
