...
![第 28 课：先 Ready，再入队](./lesson-28.png)

> 把 producer→scheduler→queue→worker 的四棒接力写成发布协议，并区分 queue ACK 与 payload ACK。

## 零基础先看这里

- **它在解决什么：**为什么任务要准备齐全后，才能放进待办队列？
- **把它想成：**餐单只有在食材齐全后才交给厨师，否则会占着灶台干等。
- **这次先不用懂：**可先忽略 queue ACK 与 payload ACK 的编码方式。

## 本课结论与证据状态

- **一句话结论：**任务必须在依赖闭合后才对 worker 可见。
- **证据状态：**SCHEDULER PROPOSAL · SOURCE-MAPPED
...
1. 先不看上文，用自己的话回答：为什么任务要准备齐全后，才能放进待办队列？
2. 再对照本课结论：任务必须在依赖闭合后才对 worker 可见。
3. 根据 `SCHEDULER PROPOSAL · SOURCE-MAPPED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/ready-before-queue/)
- [← 上一课 · 第 27 课：Ready 不是一个数字](../lesson27/)
- [下一课 · 第 29 课：把 Barrier 编译成 Ready 调度 →](../lesson29/)
