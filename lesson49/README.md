...
# 第 49 课｜两岛 P0：怎样把一个想法变成可证伪候选？

![第 49 课：两岛 P0：怎样把一个想法变成可证伪候选？](./two_island_falsifiable_p0_16x9.png)

> 以 Attention island + MLP island 为例，冻结工作量、seam、resource manifest 与失败条件。

## 零基础先看这里

- **它在解决什么：**怎样把“两段会更快”变成能验证的方案？
- **把它想成：**比赛前先写清路线、计时方法和淘汰条件；跑完后才知道想法是否真的成立。
- **这次先不用懂：**可先忽略 launch 数量、CUBIN 和 Graph census 实现。

## 本课结论与证据状态

- **一句话结论：**先写清候选怎样失败，再开始实现。
...
1. 先不看上文，用自己的话回答：怎样把“两段会更快”变成能验证的方案？
2. 再对照本课结论：先写清候选怎样失败，再开始实现。
3. 根据 `PROPOSED · UNMEASURED`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/lesson-49-two-island-p0/)
- [← 上一课 · 第 48 课：什么时候应该切开 Megakernel？](../lesson48/)
- [下一课 · 第 50 课：2 CTA/SM 要打开哪五把锁？ →](../lesson50/)
