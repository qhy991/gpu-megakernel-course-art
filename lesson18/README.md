...
# 第 18 课｜把 128 KiB 大门拆成 8 个 Page-ready 小门

![第 18 课：把 128 KiB 大门拆成 8 个 Page-ready 小门](./lesson-18.png)

> 解释 page-granular weight pipeline 如何在总搬运不变时，让已到达权重页对应的 warp 提前开工。

## 零基础先看这里

- **它在解决什么：**为什么把大任务拆小能减少等待？
- **把它想成：**像餐厅一道菜做好就先上，不必等八道菜全部完成才开席。
- **这次先不用懂：**可先忽略 128 KiB 的来源和具体同步指令。

## 本课结论与证据状态

- **一句话结论：**优化的是等待依赖粒度，不是权重字节数。
...
1. 先不看上文，用自己的话回答：为什么把大任务拆小能减少等待？
2. 再对照本课结论：优化的是等待依赖粒度，不是权重字节数。
3. 根据 `MEASURED · LEGACY DIRTY SOURCE`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/page-granular-pipeline/)
- [← 上一课 · 第 17 课：时间花在哪里，不等于时间能省多少](../lesson17/)
- [下一课 · 第 19 课：Split-KV：把 8 个长任务拆成 128 个短任务 →](../lesson19/)
