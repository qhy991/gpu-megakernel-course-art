...
# 第 34 课｜从源码到 B200 的四层证据链

![第 34 课：从源码到 B200 的四层证据链](./lesson-34.png)

> 连接 Source、PTX、SASS、Runtime，并解释 JIT cache key 或 binary provenance 断链为何使结论失效。

## 零基础先看这里

- **它在解决什么：**为什么源码、机器代码和运行结果必须来自同一次构建？
- **把它想成：**菜谱、出炉照片和品尝记录必须对应同一炉面包，否则证据无法互相证明。
- **这次先不用懂：**可先忽略 PTX、SASS 的指令细节和哈希算法。

## 本课结论与证据状态

- **一句话结论：**四层都有证据仍不够；它们必须属于同一个 exact CUBIN。
- **证据状态：**PROVENANCE AUDIT · REAL CACHE GAP
...
1. 先不看上文，用自己的话回答：为什么源码、机器代码和运行结果必须来自同一次构建？
2. 再对照本课结论：四层都有证据仍不够；它们必须属于同一个 exact CUBIN。
3. 根据 `PROVENANCE AUDIT · REAL CACHE GAP`，说出这条结论能证明什么、不能外推什么。

## 继续学习

- [在线阅读本课](https://qhy991.github.io/gpu-megakernel-course-art/lessons/source-to-b200-evidence/)
- [← 上一课 · 第 33 课：双 CTA Poison Litmus](../lesson33/)
- [下一课 · 第 35 课：实验档案袋：怎样保存一个可复核结论 →](../lesson35/)
